...
DWORD WINAPI CallYARA(LPVOID lpParameter)
{
	...
	GetProcessPath((UINT64) lpParameter, path);
	...
	/* Create Command Line */
	char cmd[MAX_STR];
	sprintf_s(cmd, MAX_STR, "\"%s\" -c -w \"%s\" \"%s\"", YARA_BINARY_NAME, YARA_RULES_PATH, path);
...
DWORD WINAPI CallYaraFromEngine(LPVOID lpParameter)
{
	UINT64 pid;
	char path[MAX_STR];

	pid = (UINT64)lpParameter;

	printf("[CallYaraFromEngine] Analysing process Id: %lu\r\n", pid);

	GetProcessPath(pid, path);
	string processPath(path);

	YaraRansEngine yaraEngine;
	yaraEngine.setYaraRuleFile(YARA_RULES_PATH);
	yaraEngine.InitializeYara();

	if (processPath.size() > 0) {
		bool isRansomware = yaraEngine.YaraScanFile(processPath);
		if (isRansomware) {
			printf("[CallYaraFromEngine] File identified as RANSOMWARE: %s (%lu)\r\n", processPath.c_str(), pid);
		}
		else {
			printf("[CallYaraFromEngine] File identified as GOODWARE: %s (%lu)\r\n", processPath.c_str(), pid);
		}
	}
...
int main()
{
	...
	while (FOREVER) {
		...
		{
			printf("[NastywareConsoleApp] Created: %lu\n", msg.pid);
			/* Avoid Blocking */
			CreateThread(NULL, 0, CallYaraFromEngine, (LPVOID) msg.pid, 0, NULL);
		}
	}
	return FINISHED;
}
